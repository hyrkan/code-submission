    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('quizzes.show', $quiz->id) }}">{{ $quiz->name }}</a></li>
...
<form method="POST" action="{{ route('quizzes.update', $quiz->id) }}" id="quizForm">
    @csrf
    @method('PUT')
    ...
    <div class="card border-top border-0 border-4 border-primary mb-4">
        <div class="card-body p-4">
            <div class="card-title d-flex align-items-center">
                ...
                <h5 class="mb-0 text-primary">{{ __('Edit Quiz') }}</h5>
            </div>
            <hr>
            <div class="row g-3">
                <div class="col-md-8">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('language') is-invalid @enderror" id="language" name="language" required>
                        <option value="python" {{ old('language', $quiz->language) == 'python' ? 'selected' : '' }}>Python</option>
                        <option value="java" {{ old('language', $quiz->language) == 'java' ? 'selected' : '' }}>Java</option>
                        <option value="javascript" {{ old('language', $quiz->language) == 'javascript' ? 'selected' : '' }}>JavaScript</option>
                        <option value="c" {{ old('language', $quiz->language) == 'c' ? 'selected' : '' }}>C</option>
                        <option value="cpp" {{ old('language', $quiz->language) == 'cpp' ? 'selected' : '' }}>C++</option>
                        <option value="php" {{ old('language', $quiz->language) == 'php' ? 'selected' : '' }}>PHP</option>
                    </select>
                    @error('language')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                </div>
                <div class="col-md-4">
                    ...
                    @error('time_limit')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                </div>
                <div class="col-12">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('year_id') is-invalid @enderror" id="year_id" name="year_id">
                        ...
                        @foreach ($years as $year)
                            <option value="{{ $year->id }}" {{ old('year_id', $quiz->year_id) == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                        @endforeach
                    </select>
                    @error('year_id')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('section_id') is-invalid @enderror" id="section_id" name="section_id">
                        ...
                        @foreach ($sections as $section)
                            <option value="{{ $section->id }}" data-year-id="{{ $section->year_id }}" {{ old('section_id', $quiz->section_id) == $section->id ? 'selected' : '' }}>{{ $section->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="datetime-local" class="form-control @error('scheduled_at') is-invalid @enderror" id="scheduled_at" name="scheduled_at" value="{{ old('scheduled_at', $quiz->scheduled_at?->format('Y-m-d\TH:i')) }}">
                    @error('scheduled_at')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                </div>
                <div class="col-12">
    ...
    <div class="card border-top border-0 border-4 border-success mb-4">
        <div class="card-body p-4">
            ...
            <div id="noItemsMsg" class="text-center py-4 text-muted" style="display:none;">
    ...
    <div class="d-flex gap-2 mb-4">
        <button type="submit" class="btn btn-primary px-5"><i class="bx bx-save"></i> {{ __('Update Quiz') }}</button>
        <a href="{{ route('quizzes.show', $quiz->id) }}" class="btn btn-light px-5">{{ __('Cancel') }}</a>
    </div>
</form>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addItem(data) {
        ...
        +         '<input type="text" class="form-control" name="items[' + idx + '][title]" required placeholder="e.g. Reverse a String" value="' + (data.title || '') + '">'
        +       '</div>'
        +       '<div class="col-md-3">'
        ...
        +         '<input type="number" class="form-control" name="items[' + idx + '][points]" required min="0" value="' + (data.points || '10') + '">'
        +       '</div>'
        +       '<div class="col-12">'
        +         '<label class="form-label">Problem Description</label>'
        +         '<textarea class="form-control" name="items[' + idx + '][description]" rows="3" placeholder="Describe the coding challenge...">' + (data.description || '') + '</textarea>'
        +       '</div>'
        +       '<div class="col-md-4">'
        +         '<label class="form-label">Sample Input</label>'
        +         '<textarea class="form-control font-monospace" name="items[' + idx + '][sample_input]" rows="2">' + (data.sample_input || '') + '</textarea>'
        +       '</div>'
        +       '<div class="col-md-4">'
        +         '<label class="form-label">Sample Output</label>'
        +         '<textarea class="form-control font-monospace" name="items[' + idx + '][sample_output]" rows="2">' + (data.sample_output || '') + '</textarea>'
        +       '</div>'
        +       '<div class="col-md-4">'
        +         '<label class="form-label">Expected Output</label>'
        +         '<textarea class="form-control font-monospace" name="items[' + idx + '][expected_output]" rows="2">' + (data.expected_output || '') + '</textarea>'
        +       '</div>'
        +       '<div class="col-12">'
        +         '<label class="form-label">Coding Standards & Guidelines</label>'
        +         '<textarea class="form-control" name="items[' + idx + '][coding_standards]" rows="3" placeholder="e.g. Use proper variable naming, include comments...">' + (data.coding_standards || '') + '</textarea>'
        +       '</div>'
        +       '<div class="col-12">'
        +         '<label class="form-label">Grading Criteria</label>'
        +         '<textarea class="form-control" name="items[' + idx + '][grading_criteria]" rows="3" placeholder="e.g. Correctness (40%), Code style (20%)...">' + (data.grading_criteria || '') + '</textarea>'
        +       '</div>'
        +     '</div>'
        +   '</div>'
        + '</div>';
    ...
    // Load existing items
    var existingItems = @json($quiz->items->values());
